// -----------------------------------------------------------------------
    ...
    // MdA-Zeile bleibt; Name wird durch Hinweis ersetzt (Person steht zusätzlich in Senat-CSV)
    const MDA_SENAT_MASK_TEXT =
      "Das Mitglied des Abgeordnetenhauses aus diesem Wahlkreis wurde nach der Wahl in den Senat berufen.";
    ...
    const aghExecutiveNames = new Set();
    const aghExecutiveOverrideNames = new Set();
    ...
    let departmentClusterLabels = {};
    let departmentKeywordToCluster = {};
    ...
    function registerRoleAliases(canonical, labels) {
      ...
      labels.forEach(label => {
        ...
        if (key) BVV_ROLE_SOURCE_MAP.set(key, canonical);
      });
    }
    ...
    const departmentClusterLabelsUrl = "./data-sources/department-cluster-labels.json";
    const departmentKeywordToClusterUrl = "./data-sources/department-keyword-to-cluster.json";
    ...
    */
    ...
    async function loadDepartmentMaps() {
      ...
      departmentMapsLoading = Promise.all([
        fetch(departmentKeywordLabelsUrl).then(res => res.ok ? res.json() : {}),
        fetch(departmentClusterLabelsUrl).then(res => res.ok ? res.json() : {}),
        fetch(departmentKeywordToClusterUrl).then(res => res.ok ? res.json() : {})
      ]).then(([keywordLabels, clusterLabels, keywordToCluster]) => {
        departmentKeywordLabels = keywordLabels || {};
        departmentClusterLabels = clusterLabels || {};
        departmentKeywordToCluster = keywordToCluster || {};
        ...
        departmentClusterLabels = {};
        departmentKeywordToCluster = {};
        departmentMapsLoaded = true;
        departmentMapsLoading = null;
      });
    ...
    function interpretDepartment(person) {
      ...
      const clusters = Array.isArray(person?.department_clusters) && person.department_clusters.length > 0
        ? person.department_clusters
        : [...new Set(
          keywords
            .map((k) => departmentKeywordToCluster[k])
            .filter(Boolean)
        )];

      return {
        keywords,
        keywordLabelsRendered: keywords.map((k) => departmentKeywordLabels[k] ?? k),
        clusters,
        clusterLabelsRendered: clusters.map((c) => departmentClusterLabels[c] ?? c)
      };
    }

    function getDepartmentClusterText(entry) {
      if (!entry) return "";
      const rawKeywords = Array.isArray(entry.department_keywords) ? entry.department_keywords : [];
      const rawClusters = Array.isArray(entry.department_clusters) ? entry.department_clusters : [];
      const keywords = rawKeywords.map(normalizeDepartmentKeyword).filter(Boolean);
      const clusters = rawClusters.length > 0
        ? rawClusters
        : [...new Set(
          keywords
            .map((k) => departmentKeywordToCluster[k])
      ...
      const labels = clusters.map((c) => departmentClusterLabels[c] ?? c).filter(Boolean);
      return labels.join(" · ");
    }
    ...
    function getBvvRoleCanonical(value) {
      const raw = String(value || "").trim();
      if (!raw) return "";
    ...
    function buildBvvPersonCard(person, options) {
      ...
      const leadershipMembership = pickLatestMembership(
        activeBvv.filter(m => {
          const canonical = getBvvRoleCanonical(m.role);
      ...
      const role = normalizeBvvRole(primaryBvvMembership?.role, gender);
      ...
      memberships.forEach(membership => {
        ...
        const committeeInfo = resolveCommitteeFromLabels(getCommitteeLabels(membership, org));
        const orgLabel = committeeInfo ? formatCommitteeShort(committeeInfo) : orgName;
        const roleLabel = String(membership?.role || "").trim();
        if (!orgLabel && !roleLabel) return;
        const committeeLike = orgLooksLikeCommittee(org) || /ausschuss|unterausschuss|beirat|kommission/i.test(orgName) || /ausschuss|unterausschuss|beirat|kommission/i.test(roleLabel);
        if (!committeeInfo && committeeLike) {
        ...
        if (line) membershipMeta.push({ org: orgLabel, role: roleLabel, text: line, committee: committeeInfo });
      });
      ...
      const activeFactionChair = activeFaction.filter(m => getBvvRoleCanonical(m.role) === "fraktionsvorsitz");
    ...
    function runBvvPersonCardTests() {
      ...
      const orgById = new Map([
        ["bvv", { id: "bvv", name: "BVV" }],
        ["comm", { id: "comm", name: "Ausschuss Finanzen" }],
        ["frac", { id: "frac", name: "CDU", shortName: "CDU" }]
      ]);
      ...
      try {
        ...
        persons.forEach(person => {
          ...
          const hasFactionChair = memberships.some(m => {
            const orgId = getOparlOrgId(m.organization);
            return factionOrgIds.has(orgId) && isActiveMembership(m, today) && getBvvRoleCanonical(m.role) === "fraktionsvorsitz";
          });
          if (hasFactionChair) {
        ...
        Papa.parse(bvvExecutiveCsvUrl, {
          ...
          complete: (result) => {
            ...
            (result.data || []).forEach(row => {
              ...
              const clustersRaw = row.department_clusters || row.department_cluster || row.DepartmentClusters || row.DepartmentCluster || "";
              const departmentKeywords = parseDepartmentKeywords(keywordsRaw);
              const departmentClusters = parseDepartmentKeywords(clustersRaw);
              ...
              const entry = {
                ...
                department_clusters: interpreted.clusters,
                department_cluster_labels: interpreted.clusterLabelsRendered
              };
        ...
        Papa.parse(senateExecutiveCsvUrl, {
          ...
          complete: (result) => {
            ...
            (result.data || []).forEach(row => {
              ...
              if (nameKey) aghExecutiveNames.add(nameKey);
            });
            resolve();
          },
          error: (err) => {
    ...
    function collectAghEntries(context) {
      ...
      const mdANamesInContext = (() => {
        ...
        const addFromList = (list) => {
          (list || []).forEach(e => {
            const k = normalizePersonName(e?.name);
            if (k) s.add(k);
          });
        };
        if (context?.awk) {
          addFromList(aghByAwk.get(context.awk));
        } else if (context?.bez) {
          const awks = aghByBez.get(context.bez);
          if (awks) {
            awks.forEach(awk => addFromList(aghByAwk.get(awk)));
          }
        }
        return s;
      })();
      const markMdASenatMask = (entry) => {
        if (entry.isExecutive) return entry;
        const key = normalizePersonName(entry?.name);
        if (key && aghExecutiveNames.has(key)) {
          aghExecutiveOverrideNames.add(entry?.name || "");
          return { ...entry, mdaSenatMask: true };
        }
        return entry;
      };
      const execEntriesDeduped = execEntries.filter(e => {
        const key = normalizePersonName(e?.name);
        if (!key) return true;
        if (mdANamesInContext.has(key)) return false;
        return true;
      });
      if (context?.awk) {
        const list = aghByAwk.get(context.awk) || [];
        return [...list.map(markMdASenatMask), ...execEntriesDeduped];
      }
      if (context?.bez) {
        const entries = [];
        const awks = aghByBez.get(context.bez);
        if (awks) {
          awks.forEach(awk => {
            ...
            list.forEach(entry => pushUnique(entries, markMdASenatMask(entry)));
          });
        }
        execEntriesDeduped.forEach(entry => pushUnique(entries, entry));
        return entries;
      }
      return execEntriesDeduped;
    }
        ...
        if (lvl === "bezirk") {
          ...
          if (entries.length > 0) {
            const orderedEntries = [...entries].sort((a, b) => {
              ...
              const isFactionChairB = b?.faction_membership && getBvvRoleCanonical(b.faction_membership.role) === "fraktionsvorsitz";
            ...
            orderedEntries.forEach((entry, entryIndex) => {
              ...
              if (Array.isArray(entry.committee_memberships)) {
                entry.committee_memberships.forEach(membership => {
                  const canonical = getBvvRoleCanonical(membership?.role);
              ...
              if (Array.isArray(entry.oparl_memberships)) {
                entry.oparl_memberships.forEach(item => {
                  ...
                  const committeeInfo = item?.committee || (orgText && isLikelyCommitteeLabel(orgText) ? resolveCommitteeFromLabels([orgText]) : null);
              ...
              const metaHtml = metaLines.length
                ...
                : "";
              const clusterText = getDepartmentClusterText(entry);
              const clusterHtml = clusterText ? `<div class="rep-role rep-department">${clusterText}</div>` : "";
          ...
          const entries = filterEntriesByParty(collectAghEntries(context));
          if (entries.length > 0) {
            ...
            const lists = entries.filter(e => !e.isExecutive && e.art !== "Direkt").sort((a, b) => stripAcademicTitles(a.name).localeCompare(stripAcademicTitles(b.name), "de"));
            const executives = entries.filter(e => e.isExecutive).sort((a, b) => stripAcademicTitles(a.name).localeCompare(stripAcademicTitles(b.name), "de"));
            const ordered = direct.concat(lists, executives);
            const execStartIndex = direct.length + lists.length;
            ordered.forEach((entry, index) => {
              const isExecutive = entry.isExecutive === true;
              ...
              const role = isExecutive
                ...
                : (isList
                  ...
                  : (() => {
                    const parts = ["Abgeordnete/r (MdA)", "Direktmandat"];
                    return parts.join(" · ");
                  })());
              const clusterText = getDepartmentClusterText(entry);
              const clusterHtml = clusterText ? `<div class="rep-role rep-department">${clusterText}</div>` : "";
              const nameDisplay = entry.name;
              const nameClass = "rep-name";
              const senatNoteHtml = entry.mdaSenatMask
                ? `<div class="rep-name mda-senat-mask">${entry.name} aus diesem Wahlkreis wurde nach der Wahl in den Senat berufen.</div>`
                : "";
              html += `<div class="rep-card agh-entry${isList ? " list-entry" : ""}${isExecDivider ? " exec-divider" : ""}">
            <div class="rep-level">${labelText}</div>
            <div class="rep-name-line">
          ...
          const entries = filterEntriesByParty(collectBundestagEntries(context));
          if (entries.length > 0) {
          ...
          const entries = filterEntriesByParty(euCitywide);
          if (entries.length > 0) {
            entries.forEach(entry => {
              const partyLabel = normalizePartyLabel(entry.party);
              html += `<div class="rep-card">
    ...
    async function initData() {
      try {
        ...
        if (aghExecutiveOverrideNames.size) {
          console.warn("MdA mit Senatsrolle (Name maskiert, keine doppelte Senat-Karte):", Array.from(aghExecutiveOverrideNames).sort());
        }
